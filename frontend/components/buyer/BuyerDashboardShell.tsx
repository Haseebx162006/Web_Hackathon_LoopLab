...
const NAV_ITEMS = [
  { href: '/buyer-dashboard', label: 'Order History', icon: IoBagHandleOutline },
  { href: '/buyer-dashboard/messages', label: 'Messages', icon: IoChatbubbleEllipsesOutline },
  { href: '/buyer-dashboard/wishlist', label: 'My Wishlist', icon: IoHeartOutline },
  { href: '/buyer-dashboard/profile', label: 'Account Profile', icon: IoPersonOutline },
] as const;
...
const BuyerDashboardShell = ({ children }: BuyerDashboardShellProps) => {
  ...
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(() => {
    ...
    return localStorage.getItem('buyer_sidebar_collapsed') === 'true';
  });

  const toggleSidebar = () => {
    const newState = !isSidebarCollapsed;
    setIsSidebarCollapsed(newState);
    localStorage.setItem('buyer_sidebar_collapsed', String(newState));
  };
        ...
        >
            ...
            <Link href="/" className="flex items-center gap-3 overflow-hidden">
              ...
              {!isSidebarCollapsed && (
                <div className="space-y-0.5 animate-fade-in-up">
                  ...
                  <p className="text-sm font-light tracking-tight text-black">Buyer Hub</p>
                </div>
              )}
            </Link>
          </div>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {!isSidebarCollapsed && (
                    <span className="text-[13px] font-light tracking-wide animate-fade-in-up">
                      {item.label}
                    </span>
                  )}
                  {isSidebarCollapsed && (
          ...
          <div className="mt-auto space-y-2 border-t border-zinc-100/50 pt-5">
            ...
            >
              ...
              {!isSidebarCollapsed && (
                <span className="text-[13px] font-light tracking-wide animate-fade-in-up">Back to Store</span>
              )}
            </Link>
            <button
              type="button"
            ...
            >
              ...
              {!isSidebarCollapsed && (
                <span className="text-[13px] font-light tracking-wide animate-fade-in-up">Logout</span>
              )}
            </button>
          </div>
          ...
          <header className="sticky top-0 z-40 bg-white/60 px-4 py-4 backdrop-blur-2xl md:px-6 lg:px-10">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 lg:hidden">
                ...
                <p className="text-xs font-light uppercase tracking-[0.2em] text-zinc-500">Buyer Console</p>
              </div>

              <div className="hidden lg:block animate-fade-in-up">
                <p className="text-[10px] font-light uppercase tracking-[0.35em] text-zinc-400">Current User</p>
              ...
              <div className="ml-auto flex items-center gap-4 rounded-2xl glass bg-white/60 px-4 py-2 border-white/20">
                <div className="text-right hidden sm:block">
                  <p className="text-[10px] font-light uppercase tracking-[0.2em] text-zinc-400">Buyer Account</p>
          ...
          <aside className="relative z-10 h-full w-[85%] max-w-xs bg-white/90 backdrop-blur-3xl px-6 py-8 shadow-2xl animate-fade-in-up">
            ...
            <div className="mt-8 space-y-2 border-t border-zinc-100/50 pt-8">
              ...
              >
                ...
                <span className="text-sm font-light tracking-wide">Back to Store</span>
              </Link>
              <button
                type="button"
                onClick={handleLogout}
